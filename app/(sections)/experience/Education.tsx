...
const Education = () => {
  return (
    <div className="flex flex-col my-20 max-sm:p-5">
      <div className="max-w-[32rem] flex justify-center items-center mx-auto">
        <ol className="relative border-s  border-gray-200 dark:border-gray-700">
          {Data.map((data) => (
            <li className="mb-10 ms-6" key={data.name}>
              <span className="absolute flex items-center justify-center w-10 h-10 rounded-full -start-6  ring-2">
                <img src={data.logo} className="rounded-full" alt={data.name} />
              </span>
              <h3 className="flex flex-col md:flex-row mb-2 text-lg font-semibold">
                {data.name}
                <Button size="sm" variant="outline" className="md:ms-3">
                  {data.timeSpan}
                </Button>
              </h3>
